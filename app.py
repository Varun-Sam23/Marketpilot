...
@media(max-width:900px){.main .block-container{padding:1rem .9rem 3rem}.mp-title{font-size:2.15rem}.mp-sub{margin-left:2.8rem;font-size:.61rem}.mp-big{font-size:1.65rem}.mp-section,.dc-section{font-size:1.35rem}.dc-title{font-size:2.1rem}}
</style>
...
def verification_badge(value):
    return {"CORROBORATED":"✅ CORROBORATED","SINGLE_SOURCE":"⚠️ SINGLE SOURCE","UNVERIFIED":"⚠️ UNVERIFIED"}.get(value,"⚠️ UNVERIFIED")


def impact_icon(value):
    return {"POSITIVE":"↗","NEGATIVE":"↘","NEUTRAL":"→","UNKNOWN":"?"}.get(value,"→")
...
if news_intel:
    ...
    for item in news_intel[:12]:
        parts.append(f'<span class="mp-news-item"><b>{verification_badge(item.get("verification"))}</b> {html.escape(item.get("title",""))} <small>• {html.escape(item.get("publisher","Unknown publisher"))} • {html.escape(item.get("impact","NEUTRAL"))}</small></span>')
...
with morning:
    ...
    if ai.get("enabled"):
        ...
        st.write("")
        x,y,z=st.columns(3)
        for c,title,key in ((x,"🟢 Bull case","bull_case"),(y,"🟡 Base case","base_case"),(z,"🔴 Bear case","bear_case")):
            c.markdown(f'<div class="mp-scenario"><h4>{title}</h4><p>{html.escape(ai.get(key,""))}</p></div>',unsafe_allow_html=True)
...
with decision:
    ...
    st.markdown('<div class="dc-section">Decision Center</div>',unsafe_allow_html=True)
    st.markdown('<div class="dc-sub">Evidence-first framework beneath the AI thesis. This is decision support, not a trading instruction.</div>',unsafe_allow_html=True)
    ...
    with a:
        ...
        st.markdown(f'<div class="dc-score">{score}<span style="font-size:1rem;color:#7d8790"> / 100</span></div>',unsafe_allow_html=True)
        st.markdown(f'<div style="color:{"#5f9b78" if score>=65 else "#c16b63" if score<=35 else "#b69a55"};font-weight:700;margin:.35rem 0 .7rem">{html.escape(bias)}</div>',unsafe_allow_html=True)
        st.markdown(f'<div class="dc-scorebar"><div style="margin-left:{score}%;height:100%;position:relative"><div class="dc-marker"></div></div></div>',unsafe_allow_html=True)
        st.markdown(f'<div style="margin-top:.7rem;color:#c5cbd3;font-size:.78rem">Regime · {html.escape(regime)}<br>Confidence · {html.escape(confidence)}</div>',unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)
    with b:
        c1,c2,c3=st.columns(3); c1.metric("AI bias",ai.get("bias","WAIT")); c2.metric("AI confidence",ai.get("confidence","N/A")); c3.metric("Rule bias",report.get("verdict","WAIT"))
        ...
        st.markdown(f'<div style="font-family:Georgia,serif;font-size:1.1rem;line-height:1.5;color:#f2eee5;margin-top:.35rem">{html.escape(ai.get("thesis") or report.get("summary") or "No thesis available yet.")}</div>',unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)
    st.markdown('<div class="dc-section">Scenario map</div>',unsafe_allow_html=True)
    x,y,z=st.columns(3)
    for c,title,key,cls in ((x,"🟢 Bull case","bull_case","dc-bull"),(y,"🟡 Base case","base_case","dc-base"),(z,"🔴 Bear case","bear_case","dc-bear")):
        c.markdown(f'<div class="dc-card dc-case {cls}"><h4>{title}</h4><p>{html.escape(ai.get(key) or "Not available")}</p></div>',unsafe_allow_html=True)
    ...
    if report.get("levels"):
        ...
        st.dataframe(pd.DataFrame([report["levels"]]),use_container_width=True,hide_index=True)
...
with live:
    ...
    if indices.empty: st.warning("Live market data is unavailable from the public feed right now.")
        ...
        ldf=indices.copy(); ldf["ticker"]=ldf["ticker"].replace({"^NSEI":"NIFTY 50","^NSEBANK":"BANK NIFTY","^BSESN":"SENSEX","^INDIAVIX":"INDIA VIX"}); ldf["last"]=ldf["last"].round(2); ldf["change_pct"]=ldf["change_pct"].round(2)
    ...
    st.dataframe(wdf.sort_values("1D %",ascending=False),use_container_width=True,hide_index=True) if not wdf.empty else st.caption("Watchlist data is temporarily unavailable.")

with news_tab:
    st.markdown('<div class="mp-section">News intelligence</div>',unsafe_allow_html=True)
    st.markdown('<div class="mp-note">Google News is only the collection layer. Publisher identity is taken from source metadata or headline attribution. Corroborated means similar reporting across independent publisher identities; it does not prove truth.</div>',unsafe_allow_html=True)
    if news_intel:
        rows=[]
        for item in news_intel[:20]:
            rows.append({"Headline":item.get("title",""),"Verification":verification_badge(item.get("verification")),"Impact":f'{impact_icon(item.get("impact"))} {item.get("impact","NEUTRAL")}',"Affected":item.get("affected","MARKET"),"Publisher":item.get("publisher","Unknown publisher"),"Published":item.get("published",""),"Evidence":item.get("verification_detail","")})
...
with performance:
    ...
    if history:
        ...
        st.dataframe(hdf,use_container_width=True,hide_index=True); st.caption("Outcome scoring will be layered onto the journal after enough completed trading sessions are available.")
