"""
MarketPilot pre-market engine.

Generates a deterministic morning report from free/public data.
Optional Gemini enhancement can be added later with GEMINI_API_KEY.
"""
...
def nifty_levels():
    ...
    sma50 = float(c.tail(50).mean()) if len(c) >= 50 else float(c.mean())
    recent_high = float(h["High"].tail(20).max())
    recent_low = float(h["Low"].tail(20).min())
    return {
        "NIFTY close": round(last, 2),
        "Previous close": round(prior, 2),
        "20D SMA": round(sma20, 2),
        "50D SMA": round(sma50, 2),
        "20D high": round(recent_high, 2),
        "20D low": round(recent_low, 2),
    }
...
def make_verdict(levels, snapshots):
    ...
    if n and n.get("change_pct", 0) > 0:
        score += 1; reasons.append("NIFTY closed higher than the previous session.")
    elif n:
        score -= 1; reasons.append("NIFTY closed lower than the previous session.")

    if levels:
        if levels["NIFTY close"] > levels["20D SMA"]:
            score += 1; reasons.append("NIFTY is above its 20-day average.")
        else:
            score -= 1; reasons.append("NIFTY is below its 20-day average.")

    if b and b.get("change_pct", 0) > (n.get("change_pct", 0) if n else 0):
        score += 1; reasons.append("Bank Nifty showed relative strength.")
    elif b:
        reasons.append("Bank Nifty did not outperform NIFTY in the last session.")

    if v:
        if v.get("change_pct", 0) > 5:
            score -= 1; reasons.append("India VIX rose sharply; volatility risk is elevated.")
        elif v.get("change_pct", 0) < -5:
            score += 1; reasons.append("India VIX fell sharply; volatility pressure eased.")
