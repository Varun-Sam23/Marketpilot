...
.evidence-wrap{border:1px solid #202c3c;border-radius:12px;overflow:hidden;background:#0b1119}.evidence-table{width:100%;border-collapse:collapse;font-size:.73rem}.evidence-table th{text-align:left;padding:9px 10px;background:#171b25;color:#8492a4;font-weight:600;border-bottom:1px solid #263344}.evidence-table td{padding:8px 10px;border-bottom:1px solid #1d2938;color:#dce3ec;vertical-align:top}.evidence-table tr:last-child td{border-bottom:0}.status-supported{color:#64d39a;font-weight:700}.status-disputed{color:#ff7b83;font-weight:700}.status-insufficient{color:#ffd45c;font-weight:700}.evidence-num{font-variant-numeric:tabular-nums;white-space:nowrap}.evidence-headline{min-width:320px;line-height:1.35}
...
current_snapshot={"bias":str(bias),"score":score,"confidence":str(confidence),"regime":str(regime),"indices":{str(r["Index"]):float(r["Session %"]) for _,r in indices.iterrows()} if not indices.empty else {},"news":{f'{x.get("title","")}|{x.get("publisher","")}' for x in news[:20]},"watch":{str(r["Stock"]):float(r["1D %"]) for _,r in wl.iterrows()} if not wl.empty else {}}
...
if previous:
    ...
    if current_snapshot["news"]-set(previous.get("news",set())): changes.append(("NEWS",f'{len(current_snapshot["news"]-set(previous.get("news",set())))} new headline(s) surfaced',"positive"))
...
a,b=st.columns([1.7,1]); a.markdown(f'<div class="verdict"><div class="label">AI MARKET VERDICT · EVIDENCE WEIGHTED</div><div class="verdict-value {cls}">{html.escape(str(bias))} · {score_label}</div><div class="verdict-copy">{html.escape(str(thesis))}</div></div>',unsafe_allow_html=True); b.markdown(f'<div class="verdict"><div class="label">RISK POSTURE</div><div class="verdict-value" style="font-size:1.35rem">{html.escape(str(regime))}</div><div class="verdict-copy">Confidence: <b>{html.escape(str(confidence))}</b> · Supported: <b>{supported}</b> · Disputed: <b>{disputed}</b></div></div>',unsafe_allow_html=True)
...
if news:
    parts=[]
    for x in news[:14]: parts.append(f'{html.escape(x.get("title", ""))} · {html.escape(x.get("publisher", "Unknown"))}')
...
with c2:
    movers=sorted(current_snapshot["watch"].items(),key=lambda z:z[1],reverse=True); rows=''.join(f'<div class="change-item"><b>{html.escape(s)}</b> <span class="{"positive" if v>0 else "negative" if v<0 else "neutral"}">{v:+.2f}%</span></div>' for s,v in movers[:5])
...
with t3:
    rows=[]
    for x in news[:24]:
        status_value=x.get("claim_status","INSUFFICIENT EVIDENCE"); title=re.sub(r"^(🟢 SUPPORTED|🔴 DISPUTED|🟡 INSUFFICIENT EVIDENCE)\s*·\s*","",x.get("title","")); rows.append({"Status":status_value,"Headline":title,"Verification":x.get("verification","UNVERIFIED"),"Evidence":x.get("evidence_score",10),"Sources":x.get("evidence_count",0),"Publisher":x.get("publisher","Unknown")})
    if rows:
        trs=[]
        for r in rows:
            status_cls={"SUPPORTED":"status-supported","DISPUTED":"status-disputed","INSUFFICIENT EVIDENCE":"status-insufficient"}.get(r["Status"],"neutral")
            trs.append(f'<tr><td class="{status_cls}">{html.escape(str(r["Status"]))}</td><td class="evidence-headline">{html.escape(str(r["Headline"]))}</td><td>{html.escape(str(r["Verification"]))}</td><td class="evidence-num">{html.escape(str(r["Evidence"]))}</td><td class="evidence-num">{html.escape(str(r["Sources"]))}</td><td>{html.escape(str(r["Publisher"]))}</td></tr>')
