...
NIFTY50_MOVERS=["ADANIENT.NS","ADANIPORTS.NS","APOLLOHOSP.NS","ASIANPAINT.NS","AXISBANK.NS","BAJAJ-AUTO.NS","BAJFINANCE.NS","BAJAJFINSV.NS","BEL.NS","BHARTIARTL.NS","CIPLA.NS","COALINDIA.NS","DRREDDY.NS","EICHERMOT.NS","ETERNAL.NS","GRASIM.NS","HCLTECH.NS","HDFCBANK.NS","HDFCLIFE.NS","HEROMOTOCO.NS","HINDALCO.NS","HINDUNILVR.NS","ICICIBANK.NS","INDUSINDBK.NS","INFY.NS","ITC.NS","JIOFIN.NS","JSWSTEEL.NS","KOTAKBANK.NS","LT.NS","M&M.NS","MARUTI.NS","MAXHEALTH.NS","NESTLEIND.NS","NTPC.NS","ONGC.NS","POWERGRID.NS","RELIANCE.NS","SBILIFE.NS","SBIN.NS","SHRIRAMFIN.NS","SUNPHARMA.NS","TATACONSUM.NS","TATAMOTORS.NS","TATASTEEL.NS","TCS.NS","TECHM.NS","TITAN.NS","TRENT.NS","ULTRACEMCO.NS","WIPRO.NS"]
NEWS_FEEDS=["https://news.google.com/rss/search?q=India%20stock%20market%20NSE%20Nifty&hl=en-IN&gl=IN&ceid=IN:en","https://news.google.com/rss/search?q=RBI%20India%20economy%20markets&hl=en-IN&gl=IN&ceid=IN:en","https://news.google.com/rss/search?q=Indian%20stocks%20earnings%20results%20companies&hl=en-IN&gl=IN&ceid=IN:en","https://news.google.com/rss/search?q=US%20markets%20Asia%20markets%20Fed%20oil%20geopolitics&hl=en-IN&gl=IN&ceid=IN:en"]

st.markdown("""
<style>
.stApp{background:#070b12;color:#e7edf5}.block-container{max-width:1500px;padding-top:1rem;padding-bottom:2rem}.hero{display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:12px}.title{font-family:Georgia,serif;font-size:2.35rem;font-weight:700}.sub{color:#8290a3;letter-spacing:.12em;text-transform:uppercase;font-size:.64rem;margin-top:7px}.time{text-align:right;color:#8290a3;font-size:.72rem}.pill{display:inline-block;border:1px solid #2b3a4d;background:#0d131d;border-radius:999px;padding:5px 10px;font-size:.67rem;font-weight:700}.deck{display:flex;gap:9px;align-items:center;background:#0b1119;border:1px solid #202c3c;border-radius:12px;padding:8px 10px;margin:8px 0 15px;overflow-x:auto}.deck span{white-space:nowrap;font-size:.72rem;color:#cdd6e0;border-right:1px solid #263344;padding-right:10px}.deck .label{font-size:.61rem;color:#718096;text-transform:uppercase;letter-spacing:.1em}.box,.change{background:linear-gradient(145deg,#0e151f,#0b1119);border:1px solid #202c3c;border-radius:14px;padding:14px 15px;height:100%}.label,.change-label{color:#7f8da0;font-size:.61rem;text-transform:uppercase;letter-spacing:.12em}.big{font-family:Georgia,serif;font-size:1.55rem;font-weight:800;margin-top:5px}.muted{color:#8492a4;font-size:.72rem;line-height:1.4}.positive{color:#64d39a}.negative{color:#ff7b83}.neutral{color:#c5ced9}.verdict{background:linear-gradient(145deg,#101925,#0c131d);border:1px solid #2a394c;border-radius:15px;padding:17px 18px;min-height:145px;height:auto;box-sizing:border-box;overflow:hidden}.verdict-value{font-family:Georgia,serif;font-size:1.85rem;font-weight:800;margin-top:5px;line-height:1.15;overflow-wrap:anywhere}.verdict-copy{color:#b7c1ce;font-size:.84rem;line-height:1.45;margin-top:7px;overflow:hidden;overflow-wrap:anywhere;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical}.section{font-family:Georgia,serif;font-size:1.18rem;margin:16px 0 8px}.meta{float:right;font-family:Arial,sans-serif;color:#718096;font-size:.62rem;letter-spacing:.09em;text-transform:uppercase;margin-top:5px}.ticker{overflow:hidden;border:1px solid #202c3c;border-radius:11px;background:#0b1119;padding:9px;white-space:nowrap;margin:8px 0 16px}.track{display:inline-block;padding-left:100%;animation:scroll 150s linear infinite}@keyframes scroll{from{transform:translateX(0)}to{transform:translateX(-100%)}}.change{min-height:150px}.change-item{padding:7px 0;border-top:1px solid #1d2938;font-size:.76rem;line-height:1.35}.change-item:first-child{border-top:0}.change-value{font-size:.85rem;font-weight:800}.risk-row{padding:7px 0;border-top:1px solid #1d2938}.risk-row:first-child{border-top:0}.evidence-wrap{border:1px solid #202c3c;border-radius:12px;overflow:hidden;background:#0b1119}.evidence-table{width:100%;border-collapse:collapse;font-size:.72rem;table-layout:fixed}.evidence-table th{text-align:left;padding:9px 10px;background:#171b25;color:#8492a4;font-weight:600;border-bottom:1px solid #263344}.evidence-table td{padding:9px 10px;border-bottom:1px solid #1d2938;color:#dce3ec;vertical-align:top}.evidence-table tr:last-child td{border-bottom:0}.status-supported{color:#64d39a;font-weight:700}.status-disputed{color:#ff7b83;font-weight:700}.status-insufficient{color:#ffd45c;font-weight:700}.status-dot{font-size:.9rem;margin-right:5px;vertical-align:-1px}.status-cell{white-space:nowrap;font-size:.67rem}.evidence-num{font-variant-numeric:tabular-nums;white-space:nowrap}.evidence-headline{line-height:1.35;word-wrap:break-word}.evidence-headline-title{font-size:.76rem;color:#e7edf5;font-weight:600;line-height:1.4}.evidence-gist{color:#91a0b2;font-size:.68rem;line-height:1.45;margin-top:5px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.evidence-point{display:block}.evidence-point::before{content:"• ";color:#64748b}.movers-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.mover-card{background:#0b1119;border:1px solid #202c3c;border-radius:12px;padding:11px 13px}.mover-row{display:grid;grid-template-columns:1fr auto auto;gap:14px;align-items:center;padding:5px 0;border-top:1px solid #1d2938;font-size:.76rem}.mover-row:first-child{border-top:0}.mover-symbol{font-weight:700;color:#dce3ec}.mover-price{font-variant-numeric:tabular-nums;color:#aeb9c7;text-align:right}.mover-pct{font-weight:800;font-variant-numeric:tabular-nums;text-align:right}.movers-note{color:#718096;font-size:.62rem;margin-top:7px}.mover-form{margin:0!important}.mover-form button{background:transparent!important;border:0!important;color:#dce3ec!important;font-weight:700!important;text-align:left!important;padding:3px 0!important;box-shadow:none!important}.mover-form button:hover{color:#fff!important}.mover-focus{font-size:.62rem;color:#718096;margin-top:8px}
...
def watch_data(tickers):
    ...
    for ticker in tickers:
        try:
            ...
            if len(h)>=22:
                c=h.Close;last=float(c.iloc[-1]);prev=float(c.iloc[-2]);sma=float(c.tail(20).mean());rows.append({"Stock":ticker.replace(".NS",""),"1D %":(last/prev-1)*100,"5D %":(last/float(c.iloc[-6])-1)*100,"20D %":(last/float(c.iloc[-21])-1)*100,"vs 20D SMA %":(last/sma-1)*100})
        except Exception:pass
    return pd.DataFrame(rows)
...
def market_movers_data():
    rows=[]
    try:
        h=yf.download(NIFTY50_MOVERS,period="5d",interval="1d",auto_adjust=False,progress=False,threads=True);close=h["Close"] if isinstance(h.columns,pd.MultiIndex) and "Close" in h.columns.get_level_values(0) else h
    ...
    except Exception:return pd.DataFrame(columns=["Stock","Price","1D %"])
    return pd.DataFrame(rows)

def clean_news_text(value):
    text=re.sub(r"<[^>]+>"," ",str(value or ""));text=html.unescape(text);return re.sub(r"\s+"," ",text).strip()
...
if previous:
    if score is not None and previous.get("score") is not None and abs(score-previous["score"])>=3:changes.append(("Decision score",f"{previous['score']:.0f} → {score:.0f}","positive" if score>previous["score"] else "negative"))
    for name,val in current["indices"].items():
        old=previous.get("indices",{}).get(name)
        if old is not None and abs(val-old)>=0.25:changes.append((name,f"{old:+.2f}% → {val:+.2f}%","positive" if val>old else "negative"))
    for name,val in current["watch"].items():
        old=previous.get("watch",{}).get(name)
        if old is not None and abs(val-old)>=0.75:changes.append((name,f"{old:+.2f}% → {val:+.2f}%","positive" if val>old else "negative"))
    for name,val in current["movers"].items():
        old=previous.get("movers",{}).get(name)
        if old is not None and abs(val-old)>=1.0:changes.append((f"Mover {name}",f"{old:+.2f}% → {val:+.2f}%","positive" if val>old else "negative"))
st.session_state["mp_dashboard_snapshot"]=current

st.markdown(f'<div class="hero"><div><div class="title">◈ MARKETPILOT</div><div class="sub">Indian market intelligence terminal · evidence · structure · decision support</div></div><div class="time"><span class="pill">{status}</span><br>{now.strftime("%d %b %Y · %H:%M:%S IST")}</div></div>',unsafe_allow_html=True)

deck=f'<div class="deck"><span class="label">COMMAND DECK</span><span>🌅 Morning Intelligence</span><span>🎯 Decision Center</span><span>⚡ Live Market</span><span>📰 News Intelligence</span><span>📓 Performance</span></div>';st.markdown(deck,unsafe_allow_html=True)
...
st.markdown(f'<div class="verdict"><div class="label">AI MARKET VERDICT</div><div class="verdict-value {vcls}">{html.escape(str(bias))} · {score_text}</div><div class="verdict-copy">{html.escape(str(thesis))}</div></div>',unsafe_allow_html=True)

k1,k2,k3,k4=st.columns(4);k1.metric("CONFIDENCE",confidence);k2.metric("REGIME",regime);k3.metric("SUPPORTED NEWS",supported);k4.metric("EVIDENCE GAPS",insufficient)

if not news:st.info("News feed unavailable. No headlines are fabricated.")
else:
    ticker_items=[]
    for x in news[:18]:
        s=x.get("claim_status","INSUFFICIENT EVIDENCE");ticker_items.append(f"[{s}] {x.get('publisher','Unknown')} · {x.get('title','')}")
    st.markdown(f'<div class="ticker"><div class="track"> &nbsp; ◈ &nbsp; '.join(html.escape(x) for x in ticker_items)+"</div></div>",unsafe_allow_html=True)
...
if not movers.empty:
    ...
    def mover_rows(frame,prefix):
        out=[]
        for i,(_,r) in enumerate(frame.iterrows()):
            pct=float(r["1D %"]);cls="positive" if pct>0 else "negative" if pct<0 else "neutral";price=float(r["Price"]);stock=str(r["Stock"]);key=f"{prefix}_{i}_{stock}"
            c1,c2,c3=st.columns([1,1,1])
            with c1:
                if st.button(stock,key=key,use_container_width=True,help=f"Open {stock} in LIVE MARKET"):
    ...
    with left:
        st.markdown('<div class="mover-card"><div class="change-label">🟢 TOP GAINERS</div>',unsafe_allow_html=True);mover_rows(gainers,"gainer");st.markdown('<div class="movers-note">Stock · price · 1D move · click the stock symbol to open the live terminal.</div></div>',unsafe_allow_html=True)
    with right:
        st.markdown('<div class="mover-card"><div class="change-label">🔴 TOP LOSERS</div>',unsafe_allow_html=True);mover_rows(losers,"loser");st.markdown('<div class="movers-note">Stock · price · 1D move · click the stock symbol to open the live terminal.</div></div>',unsafe_allow_html=True)
...
with t1:
    ...
    with x:
        fill=max(0,min(100,score)) if score is not None else 0;st.markdown(f'<div class="box"><div class="label">DECISION RADAR</div><div class="big">{html.escape(str(bias))}</div><div class="muted">Confidence: {html.escape(str(confidence))} · Regime: {html.escape(str(regime))}</div><div style="height:5px;background:#202b39;border-radius:5px;margin-top:10px"><div style="height:100%;width:{fill}%;background:#8090a4;border-radius:5px"></div></div><div class="muted" style="margin-top:6px">Decision score <b>{score_text}</b></div></div>',unsafe_allow_html=True)
