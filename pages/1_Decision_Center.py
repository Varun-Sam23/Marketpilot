...
.dc-status{font-family:Georgia,"Times New Roman",serif;font-size:2rem;line-height:1.1;color:#f5f1e8;margin-top:.25rem}
.dc-status-valid{color:#69bf91}.dc-status-watch{color:#d1aa5d}.dc-status-bad{color:#d87570}.dc-status-wait{color:#a7afb8}
.dc-livebar{height:8px;border-radius:99px;background:rgba(255,255,255,.08);overflow:hidden;margin:.65rem 0 .4rem}.dc-livefill{height:100%;border-radius:99px}
.dc-pill{display:inline-block;padding:.3rem .6rem;border-radius:999px;border:1px solid rgba(255,255,255,.1);font-size:.68rem;letter-spacing:.08em;text-transform:uppercase;color:#cbd1d7;background:rgba(255,255,255,.035)}
...
def live_snapshot():
    """Fetch a lightweight NIFTY snapshot for live thesis health."""
    try:
        h = yf.Ticker("^NSEI").history(period="5d", interval="5m", auto_adjust=False)
        if h.empty:
            return None
        ...
        prev_session = None
        if len(h) > 1:
            daily = yf.Ticker("^NSEI").history(period="10d", interval="1d", auto_adjust=False)
            if len(daily) >= 2:
                prev_session = float(daily["Close"].iloc[-2])
        return {"last": last, "prev_session": prev_session}
    except Exception:
        return None


def thesis_health(report, live):
    ai = report.get("ai_analysis", {})
    bias = str(ai.get("bias") or report.get("verdict") or "WAIT").upper()
    levels = report.get("levels", {})
    sma20 = levels.get("20D SMA")
    prev = levels.get("Previous close")
    if prev is None:
        prev = live.get("prev_session") if live else None
    if not live or live.get("last") is None:
        return "WAITING", "No live NIFTY price is available from the public feed.", None, None, None

    price = float(live["last"])
    dist_sma = ((price / float(sma20)) - 1) * 100 if sma20 else None
    dist_prev = ((price / float(prev)) - 1) * 100 if prev else None

    if bias in {"BULLISH", "BULLISH LEAN"}:
        if sma20 and price < float(sma20) * 0.995:
            return "WEAKENING", "NIFTY is trading materially below its 20-day average; the bullish thesis is losing support.", dist_prev, dist_sma, price
        if prev and price < float(prev) * 0.995:
            return "WATCH", "NIFTY is below the previous-session close; the bullish thesis needs price confirmation.", dist_prev, dist_sma, price
        return "VALID", "Live price action is not currently contradicting the morning bullish framework.", dist_prev, dist_sma, price

    if bias in {"BEARISH", "BEARISH LEAN"}:
        if sma20 and price > float(sma20) * 1.005:
            return "WEAKENING", "NIFTY is trading materially above its 20-day average; the bearish thesis is losing support.", dist_prev, dist_sma, price
        if prev and price > float(prev) * 1.005:
            return "WATCH", "NIFTY is above the previous-session close; the bearish thesis needs renewed confirmation.", dist_prev, dist_sma, price
        return "VALID", "Live price action is not currently contradicting the morning bearish framework.", dist_prev, dist_sma, price

    return "WATCH", "The morning framework is neutral; wait for stronger live confirmation before treating the bias as directional.", dist_prev, dist_sma, price
...
st_autorefresh(interval=60_000, key="decision_center_refresh")
live = live_snapshot()
health, health_reason, dist_prev, dist_sma, live_price = thesis_health(report, live)
health_cls = {"VALID": "dc-status-valid", "WATCH": "dc-status-watch", "WEAKENING": "dc-status-bad", "WAITING": "dc-status-wait"}.get(health, "dc-status-wait")
...
st.markdown('<div class="dc-sub">A transparent evidence score sits beneath the AI thesis — and the live monitor now checks whether price action is still supporting it.</div>', unsafe_allow_html=True)
...
with left:
    ...
    st.markdown(f'<div class="dc-note">Confidence: <b style="color:#e7e2d8">{safe(confidence)}</b><br>{safe(decision.get("methodology", "Evidence-weighted framework"))}</div>', unsafe_allow_html=True)
...
st.markdown('<div class="dc-section">Live thesis health</div>', unsafe_allow_html=True)
h1, h2, h3 = st.columns([1.0, 1.2, 1.5])
with h1:
    st.markdown('<div class="dc-card">', unsafe_allow_html=True)
    st.markdown('<div class="dc-label">Current state</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="dc-status {health_cls}">{safe(health)}</div>', unsafe_allow_html=True)
    st.markdown('<div class="dc-note">Re-evaluated every 60 seconds while this page is open.</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
with h2:
    st.markdown('<div class="dc-card">', unsafe_allow_html=True)
    st.markdown('<div class="dc-label">NIFTY live</div>', unsafe_allow_html=True)
    live_text = f"{live_price:,.2f}" if live_price is not None else "—"
    st.markdown(f'<div class="dc-value">{live_text}</div>', unsafe_allow_html=True)
    if dist_prev is not None:
        st.markdown(f'<span class="dc-pill">Vs prev close {dist_prev:+.2f}%</span>', unsafe_allow_html=True)
    if dist_sma is not None:
        st.markdown(f'<span class="dc-pill">Vs 20D SMA {dist_sma:+.2f}%</span>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
with h3:
    st.markdown('<div class="dc-card">', unsafe_allow_html=True)
    st.markdown('<div class="dc-label">Why the state is what it is</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="dc-note" style="font-size:.9rem;color:#d5d9de;margin-top:.55rem">{safe(health_reason)}</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
with c3:
    invalidation = ai.get("invalidation") or decision.get("invalidation") or "No explicit invalidation condition stored yet."
    st.markdown(f'<div class="dc-card"><div class="dc-label">Invalidation</div><div class="dc-note" style="font-size:.88rem;color:#d6d9dc;margin-top:.55rem">{safe(invalidation)}</div></div>', unsafe_allow_html=True)
