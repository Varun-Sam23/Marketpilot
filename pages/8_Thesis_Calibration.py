...
st.markdown("""
<style>
.stApp{background:#070b12;color:#e7edf5}.block-container{padding-top:1.5rem;max-width:1500px}
.mp-title{font-size:2rem;font-weight:800;letter-spacing:-.03em}.mp-sub{color:#8d9aab;margin-bottom:1.2rem}
.card{background:#0d131d;border:1px solid #202b3a;border-radius:14px;padding:16px;min-height:105px}.label{color:#8290a3;font-size:.75rem;text-transform:uppercase;letter-spacing:.08em}.value{font-size:1.55rem;font-weight:750;margin-top:5px}
</style>
...
if st.button("↻ Refresh calibration"):
    st.cache_data.clear()
...
def section(title,key):
    ...
    rows=data.get(key,[])
    if not rows:
        st.caption("Not enough evaluated observations yet.")
        return
...
section("Accuracy by confidence","confidence")
section("Accuracy by directional bias","bias")
section("Accuracy by market regime","regime")
