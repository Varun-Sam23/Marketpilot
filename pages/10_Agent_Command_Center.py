...
right.markdown(f'<div class="chief"><div class="label">GOVERNANCE</div><div class="list">Conflicts reported: <b>{"YES" if ai.get("conflicts") else "NONE REPORTED"}</b></div><div class="list">No order execution: <b>ENFORCED</b></div><div class="list">AI status: <b>{html.escape(str(ai.get("status", "NOT RUN")))}</b></div><div class="list">Current market state: <b>{html.escape(market_status)}</b></div></div>', unsafe_allow_html=True)
...
st.caption("MarketPilot Agent Command Center · 9 specialist agents · deterministic evidence + Chief AI adjudication · no order execution.")
